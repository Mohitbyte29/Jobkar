...
const Application = () => {
  ...
  useEffect(() => {
    ...
    const handleGetData = async () => {
      try {
        console.log(user?.id)
        const res = await axios.get(`http://localhost:4000/api/applications/${user?.id}`, { withCredentials: true });
  ...
  <div className="flex-1 p-8">
    ...
    {applications.map((application) => (
      <div key={application.id} className="bg-white p-5 rounded-xl border border-brand-border hover:shadow-sm transition-shadow">
        <div className="flex items-start justify-between">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mb-2">{application.job?.company.name}</p>
              <div className="flex flex-wrap items-center gap-y-1 gap-x-6 text-xs text-slate-500">
                <span className="flex items-center gap-1.5">
                  <i className="fa-solid fa-location-dot" /> {application.job?.location}
                </span>
                <span className="flex items-center gap-1.5">
                  <i className="fa-solid fa-sack-dollar" /> ₹{application.job?.salaryMin/100000} — ₹{application.job?.salaryMax/100000} LPA
                </span>
                <span className="flex items-center gap-1.5">
                  <i className="fa-solid fa-briefcase" /> {application.job?.type}
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-end gap-3">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-amber-50 text-amber-600 text-xs font-semibold rounded-md border border-amber-100 flex items-center gap-1.5">
                <i className="fa-solid fa-circle text-[6px]" /> {application?.status}
              </span>
            ...
            <div className="flex gap-2">
              <Link to={`/jobs/search/${application.job?.id}`} className="px-4 py-1.5 border border-brand-border text-brand-primary text-xs font-semibold rounded-lg hover:bg-slate-50 active:scale-95 transition-transform">
                View Job
              </Link>
              <Link to={`/jobs/application/review/${user?.id}/${application.job?.id}`} className="px-4 py-1.5 bg-[#635bff] text-white text-xs font-semibold rounded-lg hover:bg-brand-secondary active:scale-95 transition-transform">
                View Application
              </Link>
            </div>
          </div>
